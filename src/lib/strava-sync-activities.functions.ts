...
function matchAndUpsertReg(admin: any, reg: any, acts: StravaActivity[], ownerUserId: string) {
...
type Input = { event_id?: string; all_users?: boolean };
...
export const stravaSyncActivities = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: body }): Promise<Result> => {
    ...
    if (virtualRegs.length === 0) {
      return { matched: 0, message: "No virtual registrations" };
    }
    ...
    return { matched, total_virtual: virtualRegs.length, skipped_no_strava: skippedNoStrava, details };
  });
