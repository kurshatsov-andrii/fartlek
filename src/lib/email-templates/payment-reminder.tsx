...
const PaymentReminderEmail = ({ name, eventTitle, ticketUrl, amount }: PaymentReminderProps) => {
  ...
  return (
    <Html lang="uk" dir="ltr">
      ...
      <Preview>Нагадування: оплата участі{eventTitle ? ` у «${eventTitle}»` : ""}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Heading style={h1}>{name ? `Привіт, ${name}!` : "Привіт!"}</Heading>

          <Text style={text}>
            Нагадуємо, що для вашої реєстрації{eventTitle ? <> на змагання <strong>«{eventTitle}»</strong></> : ""} ще{" "}
            <strong>не завантажена квитанція про оплату</strong>. Без неї участь не буде підтверджена.
          </Text>
...
export const template = {
  component: PaymentReminderEmail as React.ComponentType<Record<string, unknown>>,
  subject: (data: Record<string, unknown>) =>
    data?.['eventTitle']
      ? `Нагадування про оплату: ${String(data['eventTitle'])}`
