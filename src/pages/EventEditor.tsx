  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (wfp) {
      if (!form.wfp_merchant_login.trim() || !form.wfp_secret_key.trim() || !form.wfp_merchant_domain.trim()) {
        toast.error("Заповни всі поля WayForPay (Merchant Login, Secret Key та домен мерчанта)");
        return;
      }
    }
    if (liqpay) {
    ...
    if (wfp) {
      const { error: psErr } = await supabase
        ...
        .upsert({
          ...
          wayforpay_merchant_domain: form.wfp_merchant_domain.trim().replace(/^https?:\/\//, "").replace(/\/.*$/, ""),
            ...
            {form.is_paid && paymentMethod === "wfp" && (
              <div className="space-y-3 rounded-md border border-primary/40 bg-primary/5 p-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="fartlek.lovable.app"
                ...
                <div className="rounded-md bg-background border border-border p-3 text-xs text-muted-foreground">
                  Вкажи той самий домен, який доданий у кабінеті WayForPay. Для поточного сайту це <code className="font-mono">fartlek.lovable.app</code>.
                </div>
