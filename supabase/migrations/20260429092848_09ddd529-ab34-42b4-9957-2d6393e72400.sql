DROP FUNCTION public.get_public_stats();

CREATE OR REPLACE FUNCTION public.get_public_stats()
 RETURNS TABLE(events_count integer, runners_count integer, cities_count integer, clubs_count integer, organizers_count integer, registrations_count integer)
 LANGUAGE sql
...
AS $function$
  SELECT
    ...
    (SELECT COUNT(DISTINCT e.organizer_id)::integer
     FROM public.events e
     WHERE e.status IN ('published'::public.event_status, 'completed'::public.event_status)),
