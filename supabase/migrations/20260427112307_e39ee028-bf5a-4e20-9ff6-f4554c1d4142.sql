...
AS $function$
  SELECT
    ...
    CASE WHEN public.has_role(auth.uid(), 'admin'::public.app_role) OR e.organizer_id = auth.uid() THEN p.email ELSE NULL END,
    ...
    CASE WHEN public.has_role(auth.uid(), 'admin'::public.app_role) OR e.organizer_id = auth.uid() THEN owner_p.full_name ELSE NULL END,
    CASE WHEN public.has_role(auth.uid(), 'admin'::public.app_role) OR e.organizer_id = auth.uid() THEN owner_p.email ELSE NULL END,
